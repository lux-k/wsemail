...
namespace CommonTypes
{
	...
	/// </summary>
	public class AdMessage : System.IComparable
	{
		/// <summary>
		...
		/// <summary>
		/// Compare this Ad to another one for eqaulity.  We ignore the time
		/// that the Ad was created, but nothing else.  Should we change this?
		/// </summary>
		/// <param name="obj">Ad to compare against</param>
		/// <returns>true if the same.  false otherwise.</returns>
		public override bool Equals(object obj)
		{
			// check the type
			CommonTypes.AdMessage ad;
			ad = obj as CommonTypes.AdMessage;
			...
			if ( this.AdText != ad.AdText )
				same = false;

			if ( this.Source != ad.Source )
				same = false;
		...
		{
			return this.AdText.GetHashCode() ^ this.Source.GetHashCode() ^ this.Type.GetHashCode();
		}

		/// <summary>
		/// Make a string representation of this AdMessage.
		/// </summary>
		/// <returns>AdMessage as a string</returns>
		...
		/// <summary>
		/// Compare two AdMessages together.  If they aren't identical, they are
		/// ordered lexicographically by their AdText property.
		/// </summary>
		/// <param name="obj">AdMessage to compare to</param>
		/// <returns>0 if they are equal, -1 if we are less than obj, 1 if we are
		/// greater than obj.</returns>
		public int CompareTo(object obj)
		{
			// check for type equality
			CommonTypes.AdMessage ad = obj as AdMessage;

			if ( ad == null )
				throw new ArgumentException("Argument isn't of type CommonTypes.AdMessage", "obj");
		
			// now just use the comparer
			if ( this.Equals(ad) )
			{
				return 0;
			}
			...
			{
				// it's a bit different, order by text
				return AdText.CompareTo(ad.AdText);
			}
		}
