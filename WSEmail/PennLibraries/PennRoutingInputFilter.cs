...
namespace PennLibraries 
{
	...
	public class PennRoutingInputFilter :  SoapInputFilter
	{
		...
		public override void ProcessMessage ( Microsoft.Web.Services.SoapEnvelope env ) 
		{
			...
			*/
			Utilities.LogEvent(this + " : received message\n"+env.OuterXml);
			return;
/*
			Utilities.LogEvent(this + " : received message, going to verify\n"+env.OuterXml);
			...
			Microsoft.Web.Services.Security.X509.X509Certificate signingCert = Utilities.VerifySignature(toSend);
			...
			Utilities.LogEvent(this + " : verification is good, signature stripped\n" + env.OuterXml);
*/
		}
	}
